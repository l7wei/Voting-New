...
export async function verifyTokenEdge(token: string): Promise<CustomJWTPayload | null> {
  try {
    ...
    console.error('[verifyTokenEdge] Failed:', error instanceof Error ? error.message : 'Unknown error');
    return null;
  }
}

export async function generateTokenEdge(user: { student_id: string; name: string; _id: string }): Promise<string> {
