...
開發環境下，系統提供兩種方式模擬 OAuth 登入：

#### 方式 1: 使用模擬登入頁面 (推薦)
訪問 http://localhost:3000/login 使用模擬 OAuth 登入介面：
- 輸入學號 (userid)
- 輸入姓名 (name)
- 選擇在校狀態 (inschool)
- 自動產生或手動輸入 UUID
...
管理員學號配置在 `config/admins.json` 檔案中：
...
1. **管理員設置**
   - 編輯 `config/admins.json` 設定管理員學號
   - 上傳學生清單 CSV (data/voterList.csv)

2. **建立投票活動**
   - 使用模擬登入 (http://localhost:3000/login) 以管理員身份登入
   - 進入管理員後台
   - 建立投票活動（設定名稱、時間、規則）
   - 新增候選人/選項
